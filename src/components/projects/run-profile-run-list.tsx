...
function RunProfileRunListItem({ run }: { run: RunProfileRunRecord }) {
...
export function RunProfileRunList({ runs, className }: Props) {
  return (
    <ul className={className}>
      {runs.map((run) => (
        <RunProfileRunListItem key={run.id} run={run} />
      ))}
    </ul>
  );
}
